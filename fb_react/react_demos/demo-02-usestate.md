...
切換到頂部導覽列「02 useState」。

對應原始碼：`src/demos/Demo02UseState.jsx`
...
> 對照舊版 CDN + Babel：差異只在最上方 —— Vite 版用 `import { useState } from 'react'` 取代 `const { useState } = React`，並用 `export default` 匯出元件。
