...
### 任務
實作 `@Conditional` 條件註冊，根據不同條件建立不同的 Bean。

### 程式碼

#### 自訂條件註解 `ConditionalOnDevEnvironment.java`
```java
package com.example.practice.conditional;
...
#### 條件判斷類別 `OnDevEnvironmentCondition.java`
```java
package com.example.practice.conditional;
...
public class OnDevEnvironmentCondition implements Condition {
    ...
    public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
        // 檢查是否為開發環境
        String profile = context.getEnvironment().getProperty("spring.profiles.active", "dev");
        return "dev".equals(profile);
    }
}
```

#### 條件式配置 `ConditionalConfig.java`
```java
package com.example.practice.config;
...
public class ConditionalConfig {
    ...
    public List<String> cacheService() {
        System.out.println("✅ 建立快取服務");
    ...
    public String jsonService() {
        System.out.println("✅ 建立 JSON 服務（Jackson 可用）");
        return "JSON 服務";
    }
    ...
    public String devOnlyService() {
        System.out.println("🔧 建立僅開發環境使用的服務");
        return "僅開發環境可用的服務";
    }
}
```

#### 條件式服務 `FeatureService.java`
```java
...
public class FeatureService {
    ...
    private final String jsonService;
    private final String devOnlyService;
    private final List<String> cacheService;
    
    public FeatureService(
            String defaultService,
            String jsonService,
            String devOnlyService,
            List<String> cacheService) {
    ...
    public String getFeatureStatus() {
        ...
        status.append("功能狀態：\n");
        status.append("- 預設服務: ").append(defaultService).append("\n");
        status.append("- JSON 服務: ").append(jsonService).append("\n");
        status.append("- 開發環境服務: ").append(devOnlyService).append("\n");
        status.append("- 快取服務: ").append(cacheService.isEmpty() ? "未啟用" : "已啟用").append("\n");
        return status.toString();
    }
}
```

#### 條件演示 Controller `ConditionalDemoController.java`
```java
package com.example.practice.controller;

import com.example.practice.service.FeatureService;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;
...
public class ConditionalDemoController {
    ...
    public ConditionalDemoController(FeatureService featureService) {
        this.featureService = featureService;
    }
    
    @GetMapping
    public String getConditionalInfo() {
        return featureService.getFeatureStatus();
    }
}
```
...
# 測試 API
curl http://localhost:8080/api/conditional
...
### 學習重點
- `@ConditionalOnProperty`：根據配置屬性決定是否建立 Bean
- `@ConditionalOnMissingBean`：當沒有同類型 Bean 時才建立
- `@ConditionalOnClass`：當 classpath 有特定類別時才建立
- 自訂條件：實作 `Condition` 介面來自訂條件邏輯
...
### 任務
實作 Request Scope 和 Session Scope 在 Web 應用中的使用。

### 程式碼

#### Request Scope Bean `RequestBean.java`
```java
package com.example.practice.scope;
...
public class RequestBean {
    ...
    public RequestBean() {
        this.requestId = java.util.UUID.randomUUID().toString().substring(0, 8);
        this.requestTime = LocalDateTime.now();
        System.out.println("建立 Request Bean: " + requestId);
    }
    ...
    public String getRequestTime() {
        return requestTime.format(DateTimeFormatter.ofPattern("HH:mm:ss.SSS"));
    }
    
    public String getInfo() {
        return String.format("Request ID: %s, 建立時間: %s", requestId, getRequestTime());
    }
}
```

#### Session Scope Bean `SessionBean.java`
```java
package com.example.practice.scope;
...
public class SessionBean {
    ...
    private final List<String>访问历史;
    
    public SessionBean() {
        this.sessionId = java.util.UUID.randomUUID().toString().substring(0, 8);
        this.sessionStartTime = LocalDateTime.now();
        this.访问历史 = new ArrayList<>();
        System.out.println("建立 Session Bean: " + sessionId);
    }
    
    public void addVisit(String page) {
       访问历史.add(page + " (" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss")) + ")");
    }
    
    public List<String> getVisitHistory() {
        return new ArrayList<>(访问历史);
    }
    
    public String getInfo() {
        return String.format("Session ID: %s, 開始時間: %s, 訪問次數: %d", 
            sessionId, 
...
#### Scope 演示 Controller `WebScopeDemoController.java`
```java
package com.example.practice.controller;

import com.example.practice.scope.RequestBean;
import com.example.practice.scope.SessionBean;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;
...
@RestController
@RequestMapping("/api/web-scope")
public class WebScopeDemoController {
    
    private final RequestBean requestBean;
    private final SessionBean sessionBean;
    
    public WebScopeDemoController(RequestBean requestBean, SessionBean sessionBean) {
    ...
    public Map<String, String> getRequestInfo() {
        // 每次請求都會建立新的 RequestBean
        return Map.of(
            "requestId", requestBean.getRequestId(),
            "requestTime", requestBean.getRequestTime(),
            "info", requestBean.getInfo()
        );
    }
    
    @GetMapping("/session")
    public Map<String, Object> getSessionInfo() {
        // 同一個 Session 共享 SessionBean
        sessionBean.addVisit("/api/web-scope/session");
        
        return Map.of(
            "sessionId", sessionBean.getInfo(),
            "visitHistory", sessionBean.getVisitHistory(),
            "info", "多次刷新頁面觀察訪問歷史變化"
        );
    }
    ...
    public Map<String, Object> visit() {
        sessionBean.addVisit("/api/web-scope/visit");
        
        return Map.of(
            "message", "已記錄訪問",
            "totalVisits", sessionBean.getVisitHistory().size(),
            "history", sessionBean.getVisitHistory()
        );
    }
}
```
...
### 測試
```bash
# 測試 Request Scope（每次請求都是新實例）
curl http://localhost:8080/api/web-scope/request
curl http://localhost:8080/api/web-scope/request

# 測試 Session Scope（同一 Session 共享）
curl http://localhost:8080/api/web-scope/session
curl http://localhost:8080/api/web-scope/visit
curl http://localhost:8080/api/web-scope/visit
```

### 學習重點
- Request Scope：每次 HTTP 請求建立新實例，請求結束後銷毀
- Session Scope：每個 HTTP Session 一個實例，Session 結束後銷毀
- `proxyMode = ScopedProxyMode.TARGET_CLASS`：使用 CGLIB 代理注入
...
| 理解 @Conditional 條件註冊 | □ 未完成 □ 部分完成 □ 完全完成 | |
| 能實作 Request/Session Scope | □ 未完成 □ 部分完成 □ 完全完成 | |
...
### 4. @Conditional 條件不滿足
檢查條件是否滿足：
- `@ConditionalOnProperty`：檢查配置屬性
- `@ConditionalOnClass`：檢查 classpath 類別
- `@ConditionalOnMissingBean`：檢查是否存在同類型 Bean
