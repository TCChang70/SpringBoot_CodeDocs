...
以下流程把 Day 3 的內容串成一條「可落地執行」路徑。照順序做，可以從 0 到可運作 API。
...
### Step 2：確認設定檔與 migration

1. `application.properties` 使用本機 MySQL 帳密（見 Section 8）
2. migration 檔案存在：`src/main/resources/db/migration/V1__create_books_table.sql`

預期：啟動時 Flyway 可找到 `V1__create_books_table.sql`。
...
### Step 4：依序執行 API（驗證核心流程）

建議照 Section 9 的 7 個測試跑一遍。最小執行序列如下：

1. `POST /api/books`（合法資料）
2. `POST /api/books`（重複 ISBN）
3. `GET /api/books`
4. `PUT /api/books/{id}`
5. `DELETE /api/books/{id}`
6. `GET /api/books/{id}`（確認 404）

預期狀態碼序列：`201 -> 400 -> 200 -> 200 -> 204 -> 404`
...
預期：
1. `BookServiceTest` 通過（6 個測試）
2. `BookTransactionCommitTest` 通過（2 個測試）
3. `BookTransactionRollbackTest` 通過（2 個測試）
4. 總計 10 個測試方法通過（見 Section 3.6.9）

### Step 7：完成標準（Done Definition）
...
2. CRUD API 可用
3. DTO + `@Valid` 驗證生效
4. 例外回應由 `GlobalExceptionHandler` 統一格式輸出
5. `@Transactional` commit / rollback 行為可被測試證明
6. `mvn test` 全綠
...
public class BookResponse {
    ...
    public static BookResponse from(Book book) {
        ...
        r.isbn      = book.getIsbn();
        r.price     = book.getPrice();
        r.stock     = book.getStock();
        r.category  = book.getCategory();
...
public class BookController {
    ...
    public ResponseEntity<BookResponse> create(@Valid @RequestBody BookCreateRequest req) {
        Book book = new Book(req.getTitle(), req.getAuthor(), req.getIsbn(),
    ...
    public ResponseEntity<BookResponse> update(
        ...
        Book updatedData = new Book(req.getTitle(), req.getAuthor(), req.getIsbn(),
                req.getPrice(), req.getStock(), req.getCategory());
...
class BookServiceTest {
    ...
    private Book book(String title, String isbn, String category) {
        return new Book(title, "Alice Chen", isbn, new BigDecimal("550.00"), 30, category);
    }
...
### test/.../TransactionRollbackDemoService.java
...
### test/.../BookTransactionCommitTest.java
...
class BookTransactionCommitTest {
    ...
    void create_shouldCommitToDatabase() {
        Book saved = bookService.create(
                new Book("Commit 測試書", "Tester", ISBN_CREATE, new BigDecimal("300.00"), 10, "Test"));
        assertNotNull(saved.getId());
    ...
    void delete_shouldCommitRemovalToDatabase() {
        Book book = bookService.create(
                new Book("待刪除書", "Tester", ISBN_DELETE, new BigDecimal("150.00"), 3, "Test"));
...
### test/.../BookTransactionRollbackTest.java
...
class BookTransactionRollbackTest {
    ...
    @Autowired private BookService bookService;
    @Autowired private BookRepository bookRepository;
    @Autowired private TransactionRollbackDemoService demoService;
...
本章內容較長，建議用以下順序閱讀：
1. 先看 Section 1（Controller 全貌）掌握請求流程
2. 再看 Section 3（`@Transactional`）理解交易與測試
3. 最後看 Section 4、6、7（DTO + 例外處理 + 驗證）完成 API 防呆
...
#### 3.6.3 BookServiceTest.java（CRUD + 查詢 — 自動 Rollback）
...
#### 3.6.4 查詢測試（已合併至 BookServiceTest）

`findByCategory` 與 `searchByTitle` 測試已整合至 `BookServiceTest`（見 3.6.3）的最後兩個方法，不需要獨立的查詢測試類別。
...
- [ ] 新增 `BookServiceTest.java`（參考 Section 3.6.3），驗證 6 個 CRUD + 查詢邏輯
- [ ] 新增 `BookTransactionCommitTest.java`、`TransactionRollbackDemoService.java`、`BookTransactionRollbackTest.java`（參考 Section 3.6.6、3.6.7），驗證 commit 與 rollback 行為
- [ ] 執行 `mvn test`；預期 10 個測試方法全部通過（見 Section 3.6.9）

### ✅ 預期結果驗證

此處與 Section 9（完整 Postman 測試）內容重複，建議直接依 Section 9 的 7 個測試情境逐項驗證。

最低驗收標準：
1. `POST /api/books` 成功時回傳 `201 Created`
2. `@Valid` 失敗時回傳 `400` + `errors` 陣列
3. 重複 ISBN 回傳 `400`
4. 查詢不存在資源回傳 `404`
5. `DELETE` 後再次查詢同 id 回傳 `404`

### 🔍 觀察與理解：Rollback 實驗

建議優先使用自動化測試（Section 3.6.7）驗證 rollback，避免為了手動實驗暫時修改正式程式碼。

觀察重點：
1. `RuntimeException` 預設會 rollback
2. Checked Exception 預設不 rollback
3. 加上 `rollbackFor = Exception.class` 後，Checked Exception 也會 rollback
