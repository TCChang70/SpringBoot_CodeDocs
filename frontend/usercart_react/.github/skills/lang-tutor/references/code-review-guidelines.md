...
### TypeScript
- 使用 `any` 型別 → 改用具體型別或 `unknown`
- 非空斷言 `!` 過多使用 → 改用型別守衛（Type Guard）
- `interface` 與 `type` 混用不一致 → 統一風格（物件形狀用 `interface`，聯合型別用 `type`）
- 未啟用 `strict` 模式 → 在 `tsconfig.json` 中設定 `"strict": true`
- `enum` 造成 Tree-shaking 問題 → 改用 `const enum` 或字串聯合型別
- 忽略 `Promise` 的回傳值（未 `await`）→ 加上 `await` 或明確捨棄
