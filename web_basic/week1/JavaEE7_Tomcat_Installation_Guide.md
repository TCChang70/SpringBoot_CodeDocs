...
此文件為依據原始 JavaEE7_Tomcat_Installation_Guide 的修訂版，內容以 Apache Tomcat 最新穩定版（Tomcat 11 / 10 / 9 系列）為主軸改寫，並提供在 Windows 環境下的安裝、設定與驗證步驟。教學採用繁體中文撰寫，並註明 Java EE / Jakarta EE 相容性差異與遷移建議。
...
目錄
- 版本與相容性速覽
- 在 Windows 上安裝 Tomcat（以 Tomcat 11 為示範）
- 設定 Java 環境 (JDK)
- 下載與安裝 Tomcat
...
先決條件
- 已安裝合適版本的 JDK：Tomcat 11 需要 Java 17 或更新版本（視官方說明而定）。請安裝 Oracle JDK、OpenJDK 或其他相容發行版。
...
示例：下載 Tomcat 11.0.x zip，解壓縮到您想安裝的目錄（例如 C:\apache-tomcat-11）

3) 設定環境變數（建議）

- CATALINA_HOME：指向 Tomcat 解壓目錄（例如 C:\apache-tomcat-11）。
